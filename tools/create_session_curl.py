...
def init_flow(session, guest_token):
    ...
    # att is a per-flow anti-abuse token Twitter returns as a response header
    att = response.headers.get('att') or response.headers.get('Att')
    if att:
        headers['att'] = att
        print(f"[*] Captured att token", file=sys.stderr)
